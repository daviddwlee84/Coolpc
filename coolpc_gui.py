...
file_path = os.path.join(curr_dir, "KUPC.db")
...
class NewWindow(Toplevel):
    def __init__(self, master=None):
        ...
        def db_treeview():
            ...
            cur = con.cursor()
            cur.execute("VACUUM;")
            # BUG: sqlite3.OperationalError: no such table: Stuff
            cur.execute("SELECT * FROM Stuff")
            rows = cur.fetchall()
            for row in rows:
                new_tree.insert("", "end", values=(row[0], row[1], row[2], row[3]))
            con.close()
        ...
        def db_delete():
            ...
            cur.execute(
                "DELETE FROM Stuff WHERE rowid IN(SELECT rowid FROM Stuff WHERE  name =? AND price=? AND date=? LIMIT 1)",
                (new_db_selected_name, new_db_selected_price, new_db_selected_date),
            )
        ...
        delete_btn = ttk.Button(
            self,
            text="自此資料庫中刪除",
            command=lambda: db_delete(),
            style="Toggle.TButton",
        )
        delete_btn.grid(row=1, column=0, padx=5, pady=5, sticky=tkinter.E + tkinter.W)
    ...
    for item in soup.select("#tbdy > tr:nth-child(" + str(value) + ")"):
        for opt in item.select("td:nth-child(3) > select"):
            for opt_item in opt.find_all(value=True, disabled=False):
                total_result = re.sub(
                    total_regex, subst, opt_item.text, 0, re.MULTILINE
                )
...
def db_insert_button():
    if (
        item_selected_name != ""
        and item_selected_class != ""
        and item_selected_price != ""
        and item_selected_date != ""
    ):
        con = sqlite3.connect(file_path)
        cur = con.cursor()
        cur.execute(
        ...
        cur.execute(
            "INSERT INTO Stuff (name, class, price, date)VALUES(?, ?, ?, ?)",
            (
                item_selected_name,
                item_selected_class,
                item_selected_price,
        ...
        text.config(state=tkinter.NORMAL)
        text.insert(
            END, '已將選中"' + str(item_selected_class) + '"項目\n新增歷史資料庫\n'
        )
        text.config(state=tkinter.DISABLED)
        text.yview_moveto(1)
...
style.configure(
    "Treeview", font=("Microsoft JhengHei", SMALL_FONT), rowheight=int(SMALL_FONT * 3.5)
)
...
btn1 = ttk.Button(
    tab_3,
    text="品牌小主機、AIO｜VR 虛擬",
    command=lambda: [item_crawler_thread(1, "品牌小主機、AIO｜VR 虛擬")],
)
...
btn2 = ttk.Button(
    tab_3,
    text="手機｜平板｜筆電｜穿戴",
    command=lambda: [item_crawler_thread(2, "手機｜平板｜筆電｜穿戴")],
)
...
btn4 = ttk.Button(
    tab_1, text="處理器 CPU", command=lambda: [item_crawler_thread(4, "處理器 CPU")]
)
btn4.grid(row=0, column=0, padx=2, pady=2, sticky=tkinter.E + tkinter.W)

btn5 = ttk.Button(
    tab_1, text="主機板 MB", command=lambda: [item_crawler_thread(5, "主機板 MB")]
)
btn5.grid(row=0, column=1, padx=2, pady=2, sticky=tkinter.E + tkinter.W)

btn6 = ttk.Button(
    tab_1, text="記憶體 RAM", command=lambda: [item_crawler_thread(6, "記憶體 RAM")]
)
...
btn7 = ttk.Button(
    tab_1,
    text="固態硬碟 M.2｜SSD",
    command=lambda: [item_crawler_thread(7, "固態硬碟 M.2｜SSD")],
)
...
btn8 = ttk.Button(
    tab_1,
    text="傳統內接硬碟 HDD",
    command=lambda: [item_crawler_thread(8, "傳統內接硬碟 HDD")],
)
btn8.grid(row=0, column=4, padx=2, pady=2, sticky=tkinter.E + tkinter.W)

btn12 = ttk.Button(
    tab_1, text="顯示卡 VGA", command=lambda: [item_crawler_thread(12, "顯示卡 VGA")]
)
btn12.grid(row=0, column=5, padx=2, pady=2, sticky=tkinter.E + tkinter.W)

btn15 = ttk.Button(
    tab_1, text="電源供應器", command=lambda: [item_crawler_thread(15, "電源供應器")]
)
...
btn10 = ttk.Button(
    tab_1,
    text="散熱器｜散熱墊｜散熱膏",
    command=lambda: [item_crawler_thread(10, "散熱器｜散熱墊｜散熱膏")],
)
...
btn11 = ttk.Button(
    tab_1,
    text="封閉式｜開放式水冷",
    command=lambda: [item_crawler_thread(11, "封閉式｜開放式水冷")],
)
btn11.grid(row=1, column=2, padx=2, pady=2, sticky=tkinter.E + tkinter.W)

btn14 = ttk.Button(
    tab_1, text="機殼 CASE", command=lambda: [item_crawler_thread(14, "機殼 CASE")]
)
...
btn16 = ttk.Button(
    tab_1,
    text="機殼風扇｜機殼配件",
    command=lambda: [item_crawler_thread(16, "機殼風扇｜機殼配件")],
)
btn16.grid(row=1, column=4, padx=2, pady=2, sticky=tkinter.E + tkinter.W)

btn27 = ttk.Button(
    tab_1,
    text="介面擴充卡｜專業 Raid 卡",
    command=lambda: [item_crawler_thread(27, "介面擴充卡｜專業 Raid 卡")],
)
...
btn13 = ttk.Button(
    tab_2,
    text="螢幕｜投影機｜壁掛",
    command=lambda: [item_crawler_thread(13, "螢幕｜投影機｜壁掛")],
)
...
btn9 = ttk.Button(
    tab_2,
    text="外接硬碟｜隨身碟｜記憶卡",
    command=lambda: [item_crawler_thread(9, "外接硬碟｜隨身碟｜記憶卡")],
)
...
btn17 = ttk.Button(
    tab_2,
    text="鍵盤+鼠｜搖桿｜桌+椅",
    command=lambda: [item_crawler_thread(17, "鍵盤+鼠｜搖桿｜桌+椅")],
)
...
btn18 = ttk.Button(
    tab_2,
    text="滑鼠｜鼠墊｜數位板",
    command=lambda: [item_crawler_thread(18, "滑鼠｜鼠墊｜數位板")],
)
...
btn19 = ttk.Button(
    tab_3,
    text="IP 分享器｜網卡｜網通設備",
    command=lambda: [item_crawler_thread(19, "IP 分享器｜網卡｜網通設備")],
)
btn19.grid(row=0, column=0, padx=2, pady=2, sticky=tkinter.E + tkinter.W)

btn20 = ttk.Button(
    tab_3,
    text="網路 NAS｜網路 IPCAM",
    command=lambda: [item_crawler_thread(20, "網路 NAS｜網路 IPCAM")],
)
...
btn21 = ttk.Button(
    tab_2,
    text="音效卡｜電視卡(盒)｜影音",
    command=lambda: [item_crawler_thread(21, "音效卡｜電視卡(盒)｜影音")],
)
...
btn22 = ttk.Button(
    tab_2,
    text="喇叭｜耳機｜麥克風",
    command=lambda: [item_crawler_thread(22, "喇叭｜耳機｜麥克風")],
)
...
btn23 = ttk.Button(
    tab_2,
    text="燒錄器 CD / DVD / BD",
    command=lambda: [item_crawler_thread(23, "燒錄器 CD / DVD / BD")],
)
...
btn24 = ttk.Button(
    tab_2,
    text="USB 週邊｜硬碟座｜讀卡機",
    command=lambda: [item_crawler_thread(24, "USB 週邊｜硬碟座｜讀卡機")],
)
...
btn25 = ttk.Button(
    tab_2,
    text="行車紀錄器｜USB 視訊鏡頭",
    command=lambda: [item_crawler_thread(25, "行車紀錄器｜USB 視訊鏡頭")],
)
...
btn26 = ttk.Button(
    tab_2,
    text="UPS 不斷電｜印表機｜掃描",
    command=lambda: [item_crawler_thread(26, "UPS 不斷電｜印表機｜掃描")],
)
...
btn28 = ttk.Button(
    tab_2,
    text="網路、傳輸線、轉頭｜KVM",
    command=lambda: [item_crawler_thread(28, "網路、傳輸線、轉頭｜KVM")],
)
...
btn29 = ttk.Button(
    tab_3,
    text="OS + 應用軟體｜禮物卡",
    command=lambda: [item_crawler_thread(29, "OS + 應用軟體｜禮物卡")],
)
...
Log_label = Label(root, text="Log:", font=("Microsoft JhengHei", LARGE_FONT, "bold"))
...
function_label = Label(
    root, text="功能:", font=("Microsoft JhengHei", LARGE_FONT, "bold")
)
function_label.place(x=1080, y=160)
frame3 = Frame(root)
frame3.place(x=1080, y=190, width=260, height=100)

btn_insert_to_database = ttk.Button(
    frame3, text="加入至歷史資料庫", command=lambda: [db_insert_button_thread()]
)
btn_insert_to_database.grid(
    row=0, column=0, padx=5, pady=5, sticky=tkinter.E + tkinter.W
)
btn = ttk.Button(
    frame3,
    text="開啟歷史資料庫",
    command=lambda: NewWindow(root),
    style="Toggle.TButton",
)
btn.grid(row=1, column=0, padx=5, pady=5, sticky=tkinter.E + tkinter.W)
